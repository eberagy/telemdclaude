...
echo ""
echo "2. Starting Postgres + Redis via Docker..."
...
echo ""
echo "3. Running TeleMD migrations..."
cd apps/telemd-api
pnpm db:migrate || echo "   (Migration may need env vars — run after configuring secrets)"
cd "$REPO_ROOT"

echo ""
echo "4. Seeding TeleMD database..."
cd apps/telemd-api
pnpm db:seed || echo "   (Seed may need env vars — run after configuring secrets)"
cd "$REPO_ROOT"

echo ""
echo "5. Running AgentOps migrations..."
cd apps/agentops-api
pnpm db:migrate || echo "   (Skipped — configure AGENTOPS_DATABASE_URL first)"
cd "$REPO_ROOT"
